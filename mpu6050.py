...
class MPU6050:
    ...
    def set_offsets(self, offsets):
        """
        Définit les offsets pour le gyroscope et l'accéléromètre
        - offsets: dict avec les offsets {'gx', 'gy', 'gz', 'ax', 'ay', 'az'}
        """
        self.gx_offset = offsets.get('gx', 0)
        self.gy_offset = offsets.get('gy', 0)
        self.gz_offset = offsets.get('gz', 0)
        self.ax_offset = offsets.get('ax', 0)
        self.ay_offset = offsets.get('ay', 0)
        self.az_offset = offsets.get('az', 0)


    def calibrate(self, samples=100):        
        """
        Calibre le capteur en calculant les offsets moyens
        - samples: nombre d'échantillons à lire pour le calibrage
        """
        # Lire plusieurs échantillons
        total_ax, total_ay, total_az = 0, 0, 0
        total_gx, total_gy, total_gz = 0, 0, 0
        ...
        self.ax_offset = total_ax / samples
        self.ay_offset = total_ay / samples
        self.az_offset = total_az / samples
        self.gx_offset = total_gx / samples
        self.gy_offset = total_gy / samples
        self.gz_offset = total_gz / samples 
    ...
    def read_raw_with_offsets(self):
        """
        Lit les valeurs brutes du capteur en appliquant les offsets
        Retourne: ax, ay, az (accélération), gx, gy, gz (rotation)
        """
        ax, ay, az, gx, gy, gz = self.read_raw()
        ax -= self.ax_offset
    ...
    def read_angles_with_offsets(self):
        """
        Calcule les angles roll, pitch et yaw en utilisant les valeurs avec offsets
        Retourne: roll, pitch, yaw (en degrés)
        """
        # Lire les valeurs brutes avec offsets
        ax, ay, az, gx, gy, gz = self.read_raw_with_offsets()
